...
function displayMovieDetails(details){
    ...
    <div class = "movie-info">
        ...
        <p id="recommandation-message"></p>
    </div>
    `;
    isMovieRecommended(details);
}

function isMovieRecommended(details){
    const title = details.Title;
    const imdbRating = parseFloat(details.imdbRating);
    const metascore = parseInt(details.Metascore);
    let message;
    if((imdbRating >= 7.5)||(metascore >= 75))
        message = `${title} is Recommended!`;
    else 
        message = `${title} is not necessarily Recommended.`;
    document.getElementById("recommandation-message").innerHTML = message;
}
